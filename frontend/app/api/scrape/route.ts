...
const SCRAPINGBEE_API_KEY =
    "HFVA61FAQIU0UVJ5LSHY3ANFS1P13NG1W6CVJRXKU4FH8RQGXI00X9Q6RK6H0D4NWKE990O12G7C6FD9";
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        let pinterestUrl = `https://in.pinterest.com/search/pins/?q=${encodeURIComponent(
            query
        )}`;
        ...
        let filteredImages = data.images
            .filter((img) => {
                ...
                return (
                    img.src.includes("i.pinimg.com") &&
                    (img.src.includes("236x") ||
                        img.src.includes("474x") ||
                        img.src.includes("736x")) &&
                    img.alt &&
                    img.alt.length > 10
                ); // Ensure it has a meaningful description
            })
            .map((img, index) => ({
                ...
                title: img.alt,
                sourceUrl: `https://in.pinterest.com/search/pins/?q=${encodeURIComponent(
                    query
                )}`,
                query: query,
            }));
        ...
        try {
            ...
            const storageResponse = await fetch(
                `${DJANGO_BACKEND_URL}/api/scraped-images/`,
                {
                    method: "POST",
                    headers: {
            ...
            if (storageResponse.ok) {
                ...
                console.log(
                    `Stored ${storageData.created_count} images in Django backend`
                );
            }
        } catch (storageError) {
